...
from classifier.vcop_submit_origin import CoOp
...
# 定义日志文件的路径
log_directory = 'log'
log_filename = 'console_redirect.log'
...
def parse_option():
    ...
    parser.add_argument("--train_batch", type=int, default=12, help='num_runs') # 32
    parser.add_argument("--test_batch", type=int, default=12, help='num_runs') # 32
    ...
    #incremental setting
    parser.add_argument("--num_class", type=int, default=32, help='num_class')
    parser.add_argument("--class_per_task", type=int, default=4, help='class per task')
    parser.add_argument("--num_task", type=int, default=8, help='num_task')
...
def main(args):
    ...
    for ses in range(start_sess, args.num_task):
        ...
        print('ses:',ses)
        logging.info(f"session{ses}: {task_info}")
        print(task_info)    
        ...
        logging.info(f"session {ses} acc is {acc}")
        print('acc',acc)
