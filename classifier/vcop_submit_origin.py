...
class CLIP(nn.Module):
    ...
    def forward(self, image, num_test=None, test_class=None, test=False):
        ...
        if test:
            ...
            _, postive_indices = probability.topk(k=min(self.args.text_prompt, probability.shape[1]), dim=1, largest=True) # 选择topk 个提示
            key_choose = self.text_key[postive_indices]
            text_prompt, tokenized_prompts, nc_prompts, nc_tokenized_prompts = self.prompt_learner(postive_indices)
            
            _, bottom_similar_indices = probability.topk(k=3, dim=1, largest=False)
            negetive_key_choose = self.text_key[bottom_similar_indices]
            nege_text_prompt, nege_tokenized_prompts, nege_nc_prompts, nege_nc_tokenized_prompts = self.prompt_learner(bottom_similar_indices)
            ...
            # negetive prompt loss
            nege_nc_text_features = self.text_encoder(nege_nc_prompts, nege_nc_tokenized_prompts)
            nege_nc_text_features = nege_nc_text_features / nege_nc_text_features.norm(dim=-1, keepdim=True)
            dis = nege_nc_text_features @ nege_nc_text_features.permute(1, 0)
            loss_neg_m = dis[~torch.eye(self.args.num_prompt, dtype=torch.bool, device='cuda')].abs().mean()
            return logits, image_features, key_choose, loss_m, loss_neg_m
    ...
    def fit(self, data, len_train):
        ...
        for epoch in range(self.epochs):
            for idx, (x, y) in enumerate(train_loader):
                ...
                output, ima_feat, key_choose, loss_m, loss_n = self.model(x.cuda())
                
                loss_main = F.cross_entropy(output, y.cuda())
                loss_k = cosine_loss(ima_feat,key_choose)
                loss = loss_main + 0.4*loss_k + 0.3*loss_m - 0.1 * loss_n
